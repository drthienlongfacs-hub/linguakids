...
const QA_PATH = 'data/audio-manifests/accent-practice.qa.json';
...
export async function loadAccentVoicePackQA() {
    if (!qaPromise) {
        qaPromise = fetch(resolveAccentVoiceAssetPath(QA_PATH))
            .then((response) => {
                if (!response.ok) {
                    throw new Error(`accent_voice_qa_${response.status}`);
                }
                return response.json();
            })
            .catch((error) => {
